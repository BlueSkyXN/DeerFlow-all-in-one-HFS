...
require("ARG UV_IMAGE=ghcr.io/astral-sh/uv:0.11.1" in dockerfile, "Dockerfile must use the uv version reviewed with the upstream lockfile")
...
require("uv sync --locked" in dockerfile, "Dockerfile must install the pinned upstream backend lockfile without re-resolution")
...
require("config_version: 34" in config, "managed config must match the reviewed upstream schema version")
...
require("pool_recycle: 300" in config and "command_timeout: 30" in config, "managed config must keep the reviewed database defaults")
