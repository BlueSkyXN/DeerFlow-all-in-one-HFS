...
`DEERFLOW_REF`、`APT_MIRROR`、`NPM_REGISTRY`、`UV_INDEX_URL` 是 Docker build args，不是 HF runtime Variables。Hugging Face Docker Space 不会把 Settings -> Variables 自动传给 `Dockerfile ARG`。发布 pin 必须提交在 Dockerfile 中；当前 pin 是 `0f0955bf7b2ae64ecb5099551b86049c2091a80a`。本地可以通过 `make build DEERFLOW_REF=main` 临时覆盖。
