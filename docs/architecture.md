...
3. 从 `DEERFLOW_REPO` 拉取上游 DeerFlow，默认 `https://github.com/bytedance/deer-flow.git`。
4. 检出 `DEERFLOW_REF`，默认 `main`。
...
`hfs/nginx/nginx.conf` 保留 HF 代理传入的 `X-Forwarded-Proto` 和 `X-Forwarded-Host`。这是 auth/CSRF 正常工作的关键，否则 Gateway 可能把公网 HTTPS 请求误判成容器内 HTTP，引发 `Cross-site auth request denied.`。
...
建议给 HF Space 绑定持久化 Storage 到 `/data`。否则账号、threads、uploads、生成 secret 和运行态 config 会随 Space 重建或迁移丢失。

## 升级建议

1. 先 pin `DEERFLOW_REF` 到已验证的上游 commit SHA。
2. 推送 GitHub 和 HF。
3. 等 HF runtime `raw.sha` 与 `hf/main` 对齐。
4. 检查 `/health`、`/_ops/readyz`、`/openapi.json`、`/api/v1/auth/setup-status`。
